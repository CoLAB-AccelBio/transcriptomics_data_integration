...
author: "CoLAB AccelBio"
date: "`r format(Sys.time(), '%d %B, %Y')`"
params:
  exprDir: "/Users/jacek_marzec/Library/CloudStorage/OneDrive-SharedLibraries-AssociaçãoAccelbio/AccelBio Central Hub - Projects_gastric_cancer/data/CCLE_STAD/Expression"
  exprFile: "Batch_corrected_Expression_Public_24Q2_subsetted_trans.txt"
  annotFile: "target_CCLE_STAD.txt"
  ...
  batch_rm: FLASE
  batch_col: 'Batch'
  output_dir: "/Users/jacek_marzec/Library/CloudStorage/OneDrive-SharedLibraries-AssociaçãoAccelbio/AccelBio Central Hub - Projects_gastric_cancer/data/CCLE_STAD/Expression/data_exploration_CCLE_STAD"
  results_name: "data_exploration_CCLE_STAD"
  top_genes: 500
  split: FALSE
  clust_samples: TRUE
  clust_genes: TRUE
...
##### No transformation and no filtering
} else if ( params$transform == "none" ) {
  y <- NULL
  y$transformed <- data.matrix(data)
  y$filtered.transformed <- data.matrix(data)
  y$noNorm <- data.matrix(data)
  y$norm <- data.matrix(data)
}
```
...
```{r RLE, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 8, eval=params$rle}
...
```{r RLE_batch_rm, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 8, eval=params$rle}
if ( length(targetFile$Target) > 2 ) {
