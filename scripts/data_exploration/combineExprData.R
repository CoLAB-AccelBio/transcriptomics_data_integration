...
#   transform:    Transformation method to be used when converting read counts. Available options are: "CPM" (default), "TPM" and "none"
...
#   rle:          Generate relative log expression (RLE) plot (computationally intensive). Available options are: "TRUE" (default) and "FALSE"
...
option_list = list(
  ...
  make_option("--rle", action="store", default=TRUE, type='logical',
              help="Generate relative log expression (RLE) plot"),
  make_option("--log", action="store", default=NA, type='character',
              help="Log (base 2) transform data before normalisation"),
  make_option("--top_genes", action="store", default=500, type='numeric',
